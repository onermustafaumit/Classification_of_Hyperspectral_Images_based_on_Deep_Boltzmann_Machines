...
%%%% This code also adds sparcity penalty 
 ...
 neglabstates = 1/16*(ones(numcases,numlab));
 data = round(rand(100,numdims));
 neghidprobs = 1./(1 + exp(-data*(2*vishid) - repmat(hidbiases,numcases,1)));

 epsilonw      = epsilonw/(1.000015^((epoch-1)*numbatches));
 epsilonvb      = epsilonvb/(1.000015^((epoch-1)*numbatches));
 epsilonhb      = epsilonhb/(1.000015^((epoch-1)*numbatches));
